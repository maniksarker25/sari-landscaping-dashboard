...
export function Sidebar({ onNavigate }: SidebarProps) {
  const newMessageCount = useMessagesStore((s) => s.items.filter((m) => m.status === "new").length);
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-16 shrink-0 items-center gap-2 border-b border-border px-6">
        ...
        <span className="text-sm font-semibold tracking-tight">Aurelia Admin</span>
      </div>

      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4 scrollbar-thin">
        {navItems.map((item) => {
          const Icon = item.icon;
          const badge = item.href === "/messages" && newMessageCount > 0 ? newMessageCount : null;
          return (
            <NavLink
              key={item.href}
              to={item.href}
